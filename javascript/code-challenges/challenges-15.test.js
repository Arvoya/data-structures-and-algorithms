...
const screenForNames = (arr) => {
  ...
  return arr.filter(name => name.match(regex));
};
...
let biggerThanLuke = (arr) => {
  ...
  arr.forEach(person => {
    if(person.name === 'Luke Skywalker'){
      LukeMass = parseInt(person.mass);
    }
    if(person.mass > LukeMass){
...
const detectTicTacToeWin = (board) => {
 function checkForWin(row1, col1, row2, col2, row3, col3){
 ...
 for(let i =0; i < 3; i++){
   if(checkForWin(0, i, 1, i, 2, i) || checkForWin(i, 0, i, 1, i, 2)){
     return true
   }
 }

 if(checkForWin(0, 0, 1, 1, 2, 2) || checkForWin(0, 2, 1, 1, 2, 0)){
   return true
 }
